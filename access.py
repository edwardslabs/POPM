...
def show_access(target, source):
    if target == "*":
        ...
        if config.dbtype == "SQLite":
            config.cur.execute("SELECT COUNT(*) FROM users WHERE admin = ?", [target])
            value = config.cur.fetchone()
            rows = value[0]
            if rows > 0:
                config.cur.execute("SELECT access,admin FROM users WHERE admin = ?", [target])
                for row in config.cur.fetchall():
                    account = row[1]
                    access = row[0]
            ...
            config.cur.execute("SELECT access,admin FROM users WHERE admin = %s", [target])
...
def access_level(numnick, userlist):
    ...
    if authed == 0:
        ...
        if config.dbtype == "SQLite":
            config.cur.execute("SELECT access FROM users WHERE admin = ?", [account])
        else:
