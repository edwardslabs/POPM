...
class P10Server(object):
    def signal_handler(self, signal, frame):
        ...
        self.logger(1, "[WRITE]: %sAAA Q :Shutdown received from terminal" % (config.SERVER_NUMERIC))
        config.s.send("%s SQ %s 0 :Shutdown received from terminal\n" % (config.SERVER_NUMERIC, config.SERVER_HOST_NAME))
        self.logger(1, "[WRITE]: %s SQ %s 0 :Shutdown received from terminal" % (config.SERVER_NUMERIC, config.SERVER_HOST_NAME))
        sys.exit()

    def connect(self, time):
        config.s.connect((config.HOST, config.PORT))
        config.s.send("PASS %s\n" % (config.SERVER_PASS))
        self.logger(5, "[WRITE]: PASS %s" % (config.SERVER_PASS))
        ...
        self.logger(5, "[WRITE]: SERVER %s %s %d %d J10 %s]]] :%s" % (config.SERVER_HOST_NAME, config.HOPS, time, time, config.SERVER_NUMERIC, config.SERVER_DESCRIPTION))
    ...
    def loadclient(self, nclient, time, botname, bothost, botmode, servernumeric, botdescription):
        ...
        self.logger(5, "[WRITE]: %s N %s 1 %d %s %s %s AAAAA%s %sAA%s :%s" % (config.SERVER_NUMERIC, botname, time, botname, bothost, botmode, newclient, config.SERVER_NUMERIC, newclient, botdescription))
    ...
    def joinchannel(self, time, clientid, channel):
        ...
        self.logger(5, "[WRITE]: %s B %s %d %sAA%s:o" % (config.SERVER_NUMERIC, channel, time, config.SERVER_NUMERIC, clientid))
        ...
        self.logger(5, "[WRITE]: %sAAA M %s +o %sAA%s %d" % (config.SERVER_NUMERIC, channel, config.SERVER_NUMERIC, clientid, time))

    def eob(self):
        config.s.send("%s EB\n" % (config.SERVER_NUMERIC))
        self.logger(5, "[WRITE]: %s EB" % (config.SERVER_NUMERIC))
    ...
    def gline_http(self, ip, timewo, timew, port):
        ...
        self.logger(2, "[WRITE][HTTP_CONNECT]: %s GL * +*@%s %d %d %d :%s" % (config.SERVER_NUMERIC, ip, config.DURATION, timewo, timew, newmessage))
    ...
    def gline_dnsbl(self, ip, timewo, timew, blacklist):
        ...
        config.s.send("%s GL * +*@%s %d %d %d :%s\n" % (config.SERVER_NUMERIC, ip, config.DURATION, timewo, timew, newmessage))
        self.logger(2, "[WRITE][DNSBL_FOUND]: %s GL * +*@%s %d %d %d :%s" % (config.SERVER_NUMERIC, ip, config.DURATION, timewo, timew, newmessage))
    ...
    def gline_socks(self, ip, timewo, timew, port, version):
        ...
        config.s.send("%s GL * +*@%s %d %d %d :%s\n" % (config.SERVER_NUMERIC, ip, config.DURATION, timewo, timew, newmessage))
        self.logger(2, "[WRITE][SOCKS%s_FOUND]: %s GL * +*@%s %d %d %d :%s" % (version, config.SERVER_NUMERIC, ip, config.DURATION, timewo, timew, newmessage))
    ...
    def startbuffer(self):
        ...
        while 1:
            ...
            readbuffer=temp.pop()
            for line in temp:
                self.logger(4, "[READ]: " + line)
                line=string.rstrip(line)
                line=string.split(line)
                # Get initial data from uplink #
                if(line[0] == "SERVER"):
                    if uplinkid == line[6][:2]:
                        self.logger(1, "[ERROR][FATAL]: Uplink ID matches POPM ID. Exiting")
                        sys.exit(0)
                    else:
                        uplinkid = line[6][:2]
                        uplinkname = line[1]
                ...
                if(line[1] == "N"):
                    try:
                        if(":" in line[8]):
                            ...
                            self.logger(3, "[INFO]: New userlist is " + str(userlist))
                    except IndexError:
                        try:
                ...
                if(line[0] == uplinkid and line[1] == "EB" and complete == 0):
                    config.s.send("%s EA\n" % (config.SERVER_NUMERIC))
                    self.logger(5, "[WRITE]: %s EA" % (config.SERVER_NUMERIC))
                    complete = 1

                # Check for ID collisons #
                if(line[1] == "S"):
                    if uplinkid == line[7][:2]:
                        self.logger(1, "[ERROR][FATAL]: Server %s has same ID as POPM. Exiting" % (line[2]))
                        sys.exit(0)

                # Keep alive stuff #
                if(line[0] == uplinkid and line[1] == "Z"):
                    config.s.send("%s G :%s\n" % (config.SERVER_NUMERIC, config.SERVER_HOST_NAME))
                    self.logger(6, "[WRITE]: %s G :%s" % (config.SERVER_NUMERIC, config.SERVER_HOST_NAME))
                if(line[0] == uplinkid and line[1] == "G"):
                    config.s.send("%s Z %s %s 0 %s\n" % (config.SERVER_NUMERIC, config.SERVER_HOST_NAME, line[2][1:], line[4]))
                    self.logger(6, "[WRITE]: %s Z %s %s 0 %s" % (config.SERVER_NUMERIC, config.SERVER_HOST_NAME, line[2][1:], line[4]))

                # If we were squit, then abandon ship #
                if(line[1] == "SQ" and line[2] == uplinkname):
                    self.logger(1, "[ERROR][FATAL]: Received SQUIT from uplink. Exiting")
                    sys.exit(0)
