class Server:
    def main(self):
        ...
        import dnsbl
        import settings
        setting = settings.DBSettings()
        dnsbl = dnsbl.DNSBL()
        sig = signals.Signals()
        signal.signal(signal.SIGINT, sig.signal_handler)
        accessmodule = access.UserAccess()
        cfg = config.Config()
        HOST = cfg.HOST
        PORT = cfg.PORT
            ...
            for line in temp:
                ...
                if(line[1] == "P" and line[2][:1] == "#" or line[1] == "P" and line[2] == "%sAAA" % (SERVER_NUMERIC)):
                    ...
                    if(line[3].lower() == ":.threads" and channel is True or channel is False and line[3].lower() == ":threads"):
                        if accessmodule.access_level(target, userlist) > 750:
                            try:
                        ...
                        elif accessmodule.access_level(target, userlist) <= 749:
                        ...
                        if accessmodule.access_level(target, userlist) > 0:
                            try:
                                if line[4] != False:
                        ...
                        try:
                            if line[5] != False:
                                try:
                                    ...
                                    break
                                if accessmodule.access_level(target, userlist) == 1000:
                                    if access <= 1000:
                                        accessmodule.update_access(line[4], access, target, userlist)
                                    else:
                            ...
                            try:
                                if line[4] != False:
                                    if accessmodule.access_level(target, userlist) > 0:
                                        accessmodule.show_access(line[4], target)
                            except IndexError:
                                if accessmodule.access_level(target, userlist) > 0:
                                    s.send("%sAAA O %s :Account %s has access %s.\n" % (SERVER_NUMERIC, target, accessmodule.get_acc(target, userlist), accessmodule.access_level(target, userlist)))
                        ...
                        try:
                            if line[4] != False:
                                ...
                                while i < arlen:
                                    ...
                                    i += 1
                                account = accessmodule.get_acc(target, userlist)
                                if accessmodule.access_level(target, userlist) >= 900:
                        ...
                        try:
                            if line[4] != False:
                                if accessmodule.access_level(target, userlist) >= accessmodule.get_level_req("access_set"):
                                    if setting.is_settable(line[4]) is True:
                                        try:
                                            if line[5] != False:
                                                try:
                                                    newlevel = line[5]
                                                ...
                                                if newlevel <= 1000 and newlevel >= 0:
                                                    setting.update_settings(line[4], newlevel, target)
                                                else:
                                                    s.send("%sAAA O %s :Paramater must be an integer between 0 and 1000\n" % (SERVER_NUMERIC, target))
                                        except IndexError:
                                            setting.get_set_value(line[4], target)
                                    else:
                                        s.send("%sAAA O %s :Invalid SET option.\n" % (SERVER_NUMERIC, target))
                                else:
                                    s.send("%sAAA O %s :Insufficient access.\n" % (SERVER_NUMERIC, target))
                        except IndexError:
                            if accessmodule.access_level(target, userlist) >= accessmodule.get_level_req("setters"):
                                setting.get_set(target)